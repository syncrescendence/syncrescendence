# Archive Shell Manifest: accidental-live-copy

- root: [/Users/system/Desktop/syncrescendence_accidental_live_copy_20260302-211659](/Users/system/Desktop/syncrescendence_accidental_live_copy_20260302-211659)
- top-level entries: 15

## Disposition Summary

- `constitution-pedigree`: 2
- `live-communications`: 1
- `live-executive`: 1
- `live-office`: 1
- `live-operator`: 1
- `live-pedigree`: 1
- `live-playbook`: 1
- `live-program`: 1
- `live-runtime`: 1
- `live-validated-patterns`: 1
- `operator-pedigree`: 1
- `root-explanation`: 1
- `runtime-local-state`: 1
- `unclassified`: 1

## Top-Level Entry Classification

- `.claude` (directory): `runtime-local-state` — operator-runtime family mapping
- `00-ORCHESTRATION` (directory): `unclassified` — no rule yet
- `AGENTS.md` (file): `constitution-pedigree` — directly mapped by name
- `CLAUDE.md` (file): `constitution-pedigree` — directly mapped by name
- `communications` (directory): `live-communications` — communications family mapping
- `executive` (directory): `live-executive` — executive family mapping
- `Makefile` (file): `operator-pedigree` — directly mapped by name
- `offices` (directory): `live-office` — office-playbook family mapping
- `operators` (directory): `live-operator` — operator-runtime family mapping
- `pedigree` (directory): `live-pedigree` — knowledge-pedigree family mapping
- `playbooks` (directory): `live-playbook` — office-playbook family mapping
- `program` (directory): `live-program` — program family mapping
- `README.md` (file): `root-explanation` — directly mapped by name
- `runtime` (directory): `live-runtime` — operator-runtime family mapping
- `validated-patterns` (directory): `live-validated-patterns` — operator-runtime family mapping
