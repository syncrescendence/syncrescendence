...
Brand infrastructure decomposes into two co-equal layers that share one source of truth:

**Human-Facing System**: Traditional creative outputs -- visual identity, typography, motion, websites, brand narratives, guidelines. Delivered as PDFs, Figma files, design systems. This layer has existed for decades; it is well-understood.

**Agent-Readable System**: A structured knowledge system -- the "Agent-First Brand Kit" -- comprising Markdown, YAML, and JSON files that encode positioning, voice, differentiation, audience profiles, and visual constraints. Designed for teams working with AI agents as primary producers.

The critical invariant: both layers derive from the same positioning, voice, and structure. The agent-readable layer is not a summary of the human layer; it is the same content re-expressed in machine-parseable form. Files like `brand-positioning.yaml` and `messaging-framework.md` function as single sources of truth for both the AI agent generating code and the designer refining in Figma.

The practical implication -- stated as a precondition for modern toolchain integration -- is that structured data must exist as a first-class artifact of the brand engagement, not an afterthought.

### The Agent-First Brand Kit: Chunk Architecture

The Agent-First Brand Kit is not a single file. It is a corpus of discrete knowledge chunks, each designed to be independently retrievable and processable by an AI agent.
...
**Full brand system scope**: 15-20 chunks across five categories.
...
Figma's Claude Code integration creates a bidirectional loop: structured brand files in the codebase inform AI generation in Claude Code; the output flows into Figma for human refinement; the refined design flows back to code. This loop is only functional if structured brand data exists. A brand stored exclusively as Figma files cannot enter this loop -- it is the terminal node, not a participant.
...
The end-state of well-executed agent-ready brand systems: they become the foundation for general knowledge and context, enabling rapid indexing of information packets across the organization. The brand infrastructure becomes an epistemological substrate, not just a style guide.

### The Dual-Native System as Standard Practice

The dual-native brand system -- structured agent data running in parallel with the traditional human-facing brand -- will become a baseline component of all digital brand kits. This is framed as near-term inevitability rather than leading-edge practice.

The evidence for this trajectory: AI-native startups already require brand kits that work with agents to ship fast. The 2020-era brand kit -- typography, color palette, logo variants, PDF guidelines -- is insufficient for a team where AI agents are primary producers of copy, UI, and content. The brand kit must be machine-operable, not just machine-present.

### The Craft Scarcity Principle

A counterweight to the instrumentalization argument: AI does not diminish human craft -- it makes craft more valuable by making everything else undifferentiated. When any team can generate adequate design output instantly, handcrafted outputs carry scarcity signal.
...
The Craft Scarcity Principle frames the stakes correctly: this infrastructure is not a threat to craft but its liberation. When agents handle the volume, humans can concentrate force at the craft layer where differentiation actually lives.
...
|-------------|-------------|
| `corpus/design-taste/04276.jsonl` | 18 atoms from "A Guide to Building Brands for Humans and Agents" (Emmett Shine / Little Plains) -- dual-native brand system framework, two-layer infrastructure, agent-first brand kit chunk architecture, 400-token specification, five-category taxonomy, Code-Figma-Brand loop, soul.md agent personality pattern, craft scarcity principle (Peter Steinberger), AI-without-structure-accelerates-mediocrity thesis. |
| `corpus/design-taste/04278.md` | Extraction of same -- structured breakdown of all 18 atoms with tension vectors, covering claims, concepts, frameworks, praxis hooks, and predictions. Little Plains transition from guidelines to infrastructure, Dan Batten on brand systems as epistemological substrate, dual-native as near-term standard. |
